  ...
  setup(props, { emit }) {
    ...
    watch(
      ...
      (newVisible) => {
        if (newVisible && props.nodeData) {
          ...
          formValue.pythonEnv = props.nodeData.pythonEnv || ''
    ...
    return () => (
      <NDrawer
        ...
        show-mask={false}
      >
        <NDrawerContent title='Python节点配置'>
          <NForm ref={formRef} model={formValue} rules={rules}>
            <NFormItem label='节点名称' path='taskName' required>
              <NInput v-model:value={formValue.taskName} />
            </NFormItem>

            <NFormItem label='Python环境' path='pythonEnv'>
              <NSelect
                v-model:value={formValue.pythonEnv}
                options={[
                  { label: 'Python 2.7', value: 'python2' },
                  { label: 'Python 3.6+', value: 'python3' },
                  { label: '自定义环境', value: 'custom' }
                ]}
              />
            </NFormItem>
